...
This is a fork of OCaml compiler to build Rescript compiler.
It has a tiny patch to support conditonal compilation.

If you are already familar with opam, you can install it using opam
instead.
...
If you decide to build the compiler from source, a recommended configuration is as below:
```
./configure -cc "gcc -Wno-implicit-function-declaration -fcommon" -flambda 
```
